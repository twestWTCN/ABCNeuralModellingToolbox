function [F meannpd] = construct_spectral_NP_DTFMat(data,chloc_name,chlist,fsamp,N,R,normnoise)
if isempty(N)
...
cfg           = [];
cfg.method    = 'dtf';
dtfN           = ft_connectivityanalysis(cfg, mfreqN);
    ...
    for chJ = 1:size(chloc_name,2)
        for p = 1:size(chinds{chI},1)
            ...
            for r = 1:size(chinds{chJ},1)
                chindsR = chinds{chJ};
                if chI == chJ
                    Pxy =   squeeze(mean(abs(mfreq.crsspctrm(:,p,:)),1))';
...
%                     Cxy = (Cxy./mean(nCxy));
                    ...
                    Cxy = squeeze(dtf.dtfspctrm(chI,chJ,:));
                    nCxy = squeeze(dtfN.dtfspctrm(2,1,:));
                    Cxy = (Cxy./max(nCxy));
                    xcsd(p,r,2,:) = Cxy; %.*welchwin(length(F));
                    
                    Cxy = squeeze(dtf.dtfspctrm(chJ,chI,:));
                    nCxy = squeeze(dtfN.dtfspctrm(2,1,:)); % Always same direction as no reverse
                    Cxy = (Cxy./max(nCxy));
