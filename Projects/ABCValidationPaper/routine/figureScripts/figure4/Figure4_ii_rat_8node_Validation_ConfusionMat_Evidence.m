...
for dataN = 1:3
    ...
    X = cellfun(@(x) sum(x>=R.modcomp.modEvi.epspop)./(numel(x)+1),ACCrep,'UniformOutput',0);
    X = [X{:}];
    X = 1-X;
    X = X/sum(X); % The smaller the better
    dkl = [dkl{:}];
    dklN = dkl./sum(dkl);
    
    PMOD(:,dataN) = X'; %-log10(X)'; %X'; % the more positive the better
    CMP(:,dataN) = dklN'; %log10(dklN)'; %dkl'; the more negative the worse
    ACS(:,dataN) = (-log10(X) + log10(dklN))'; % the more negative it is
    SM(:,dataN) = SimMod;
    SD(:,dataN) = SimData;
end
